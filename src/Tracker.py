...
class Tracker(CommNet):
    ...
    def __init__(self, model, model_name, loss_function, test_loader, train_loader, device):
        self.model = model
        ...
        keys = ["test_loss","test_acc","train_loss","train_acc","cons_error","train_time","test_time"]
    ...
    def evaluate(self,loader="test",output=True):
        ...
        for batch_index, (data, target) in enumerate(data_loader):
            ...
            if self.model_name == "nanoGPT":
                model_output ,loss = self.model(data,target)
    ...
    def save_history(self,file_name,COMM_WORLD_INSTANCE):
        ...
        path = "./results/cifar/"+file_name+".yaml"
    ...
    def compute_cons_error(self,comm_set,optimizer):
        ...
        for field in comm_set:
            for name in optimizer.get_names(field):
                ...
                if self.device == 'gpu' or 'cuda' in self.device:
                    ...
                    # ws_mean, w = optimizer.all_reduce(field,name)
                    ws_mean, w = optimizer.neighbor_reduce(field,name)
        ...
        if self.device == 'gpu' or 'cuda' in self.device:
            ...
            cons = optimizer.COMM.allreduce(np.mean(np.array(c_err)), op=MPI.SUM)
            self.history["cons_error"].append(float(cons))
            return cons